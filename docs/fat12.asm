...
bpb:
.bytes_per_sector:      dw 512  ; This is the size of a hardware sector and for most disks in use in the United
...
.numbers_of_FATs:       db 2    ; This is the number of copies of the FAT table stored on the disk. Typically, 
                                ; the value of this field is 2.
.root_entries:          dw 512  ; This is the total number of file name entries that can be stored in the root 
...
.small_sectors:         dw 0    ; This field is used to store the number of sectors on the disk if the size of
...
.media_descriptor:      db 0xf0 ; This byte provides information about the media being used. The following table
                                ...
                                ;   F8   --------    Fixed disk
.sectors_per_FAT:       dw 0    ; Part of the apparent disk geometry in use when the disk was formatted.
.sectors_per_track:     dw 0    ; Part of the apparent disk geometry in use when the disk was formatted.
.hidden_sectors:        dd 0    ; This is the number of sectors on the physical disk preceding the start of the 
                                ...
                                ; data areas.
.large_sectors:         dd 0    ; If the Small Sectors field is zero, this field contains the total number of 
                                ; sectors used by the FAT volume.

ebpb:
.physical_drive_number: db 0x00 ; This is related to the BIOS physical drive number. Floppy drives are numbered 
...
.current_head:          db 0    ; This is another field typically used when doing INT13 BIOS calls. The value 
                                ...
                                ;   also be run.
.signiture:             db 0x28 ; The extended boot record signature must be either 0x28 or 0x29 in order to be
                                ; recognized by Windows NT.
.id:                    dd 0    ; The ID is a random serial number assigned at format time in order to aid in 
                                ; distinguishing one disk from another.
.volume_label:  db "NEWVOLUME00"    ; This field was used to store the volume label, but the volume label is now 
                                    ; stored as a special file in the root directory.
.system_id: db "FAT12   "       ; This field is either "FAT12" or "FAT16," depending on the format of the disk.
